...
- `/links` 改为 `h-screen` 固定一屏布局，取消整页滚动
- 顶部工具栏压缩为单行（h-14），按钮 `flex-nowrap` 横向排列
- 筛选区重构：默认两行 +「更多筛选」折叠（收藏、学期）
- 下拉筛选（平台/状态/资料分类/收藏）变化后自动刷新
- 文本筛选按 Enter 或「搜索」按钮刷新
- 表格区域内部滚动 + sticky 表头
- 详情区内部滚动，固定高度不撑破页面
- 表格新增：多选、序号、原始链接（可点击跳转）、资料信息合并列
- 表格单行固定高度（44px）+ 截断
- 多选提示栏 + 禁用「批量编辑（下一阶段）」按钮

**页面布局变化：**

- 根容器 `flex h-screen flex-col overflow-hidden`
- 主体 `flex-1 min-h-0` 分配剩余高度
- 左表格 + 右详情（xl 及以上）并排，各自内部滚动

**筛选区变化：**

- 第一行：搜索 + 重置 + 搜索
- 第二行：平台、状态、资料分类、学段、年级、科目、资料年份、更多筛选
- 折叠区：收藏、学期

**表格字段变化：**

- 多选 | 序号 | 标题 | 原始链接 | 资料信息 | 平台 | 状态 | 收藏 | 创建时间 | 操作
- 资料信息合并：学段/年级/学期/科目/资料年份/资料分类

**滚动区域变化：**

- 页面无整页滚动条
- 表格 `overflow-auto` 内部滚动
- 详情 `overflow-y-auto` 内部滚动

**多选能力：**

- 全选当前页、单行勾选、取消选择、已选计数
- 未实现批量编辑（留待阶段 6B）
...
- 小屏（< xl）隐藏右侧详情面板，需后续考虑移动端详情展示
- 翻页时会清空跨页多选状态

**下一阶段建议：**

- 阶段 6B：批量编辑弹窗 + 批量 PATCH
...
- 重构 `LinkFormDialog`：分区布局、85vh 限高、内容区内部滚动、底部按钮固定
- 链接字段独占一行，长链接 input 横向滚动
- 原始输入片段默认折叠，可展开/收起
- 新增 `LinkBatchEditDialog` 批量编辑弹窗
- 表格多选栏启用「批量编辑」按钮
- 新增 `batchUpdateLinks` 客户端辅助，逐条调用现有 PATCH API
- 新增 `npm run ui:batch-edit:test` 测试脚本

**编辑弹窗变化：**

- 宽度 max-w-[760px]，高度 max-h-[85vh]
- 分区：基础信息 / 链接信息 / 学习资料信息 / 状态信息 / 备注 / 原始输入
- 编辑时平台只读；原始链接与标准链接各占一行

**批量编辑入口：**

- 选中资料后显示：已选择 X 条 | 批量编辑 | 取消选择
...
- 标题、平台、原始链接、标准链接、提取码、原始输入、创建时间、更新时间

**批量更新实现方式：**

- 不新增批量 API
- 循环 `selectedIds`，逐条 `PATCH /api/links/[id]`
- 仅提交用户勾选启用的字段；勾选后空值提交 null 表示清空
...
- 大批量选中时逐条 PATCH 较慢，后续可考虑并发或专用批量 API
**下一阶段建议：**

- 按需：Electron 桌面封装、自动备份计划、Excel 导入
...
- 重构 `LinkImportDialog`：单弹窗模式，max-w 1160px / max-h 85vh
- 弹窗内容区内部滚动，底部固定取消 / 继续导入 / 确认导入
- 粘贴区、解析摘要、预览列表 + 批量填充分块
- 预览区（65%）与批量填充区（35%）左右布局
- 预览表格单行截断 + 内部滚动
- 解析失败项、导入结果（重复/失败）可折叠展示
- 导入成功后刷新列表，保留弹窗展示结果

**弹窗布局变化：**

- 顶部标题 + 关闭
- 内容区：粘贴 → 摘要 → 左右主体 → 失败/结果
- 底部三按钮始终可见

**粘贴区变化：**

- textarea 固定 h-36，内部滚动
- 清空同时清除粘贴、预览、导入结果

**预览区变化：**

- 左侧预览列表 max-h-64 内部滚动
- sticky 表头，固定行高，长标题/链接截断
...
- 导入完成后在内容区底部展示摘要
- 重复链接 / 失败项默认折叠（≤3 条直接展开）

**手动测试结果：**

- 弹窗布局与滚动：通过
- 百度单条 / 夸克批量 / 移除 / 重复 / 继续导入：通过
- 现有 CRUD/批量编辑/导出/备份回归：通过
...
- 窄屏下左右布局会变为上下排列，预览区高度有限
- 导入完成后需手动点「继续导入」或「取消」关闭

**下一阶段建议：**

- 按需：Electron 桌面封装、Excel 导入、自动备份计划

---

## 阶段 6B-Fix：全局提示区 + 紧凑筛选区 + 紧凑表单修正

**状态：** 完成

**完成内容：**

- 新增 `usePageToast` 统一提示状态，5 秒自动消失
- 所有操作提示移至表格 footer 分页按钮左侧
- 移除顶部绿色/红色大横条提示
- 筛选区压缩为单行横向布局，固定宽度控件
- 详情区「复制链接/复制提取码」合并为「复制信息」（复制 sourceText）
- 新增/编辑弹窗改为紧凑桌面布局，短字段同行排列
...
- 同时只显示一条，新提示覆盖旧提示，5 秒后自动消失

**筛选区布局变化：**

- 搜索 + 平台 + 状态 + 分类 + 学段 + 年级 + 学期 + 科目 + 年份 + 收藏 + 重置 + 搜索，单行横向滚动

**复制信息变化：**

- 复制 sourceText；为空时提示「当前资料没有原始输入内容。」

**新增 / 编辑弹窗变化：**

- max-w 900px，平台/分类/状态/收藏同行，学习字段一行五列，链接区紧凑
...
- 窄屏筛选区需横向滚动
- footer 提示 max-w 360px 过长会截断，悬停可看完整内容

**下一阶段建议：**

- 继续阶段 6C 批量导入弹窗 UI 重构（如尚未完成）或 Electron 封装

---

## 阶段 6B-Fix-2：表格字段调整 + 操作列复制 + 筛选区高度

**状态：** 完成

**完成内容：**

- 表格列顺序调整为：选择框、序号、资料年份、标题、科目、年级、资料分类、平台、原始链接、状态、收藏、操作
- 移除表格中的资料信息、创建时间等未提及字段
- 操作列新增「复制」按钮，复制 sourceText
- 筛选区控件高度 h-7 → h-9，padding 略增
